				...
				'<div class="plupload">' +
					...
					'<div class="plupload_content">' +
						...
						'<tr class="ui-widget-header ui-widget-content plupload_filelist_footer">' +
							...
							'</td>' +
							'<td class="plupload_cell plupload_file_status"><span class="plupload_total_status">0%</span></td>' +
							'<td class="plupload_cell plupload_file_size"><span class="plupload_total_file_size">0 kb</span></td>' +
							'<td class="plupload_cell plupload_file_action"></td>' +
						'</tr>' +


                        '<tr class="ui-widget-header ui-state-default plupload_progress_footer">' +
							'<td colspan="4">' +
    ...
    $.widget("ui.plupload", {
        ...
        _create: function () {
            ...
            uploader.bind('Init', function (up, res) {
                ...
                var usingRuntime = _('Using runtime: ') + (self.runtime = res.runtime);
                $('.plupload_header', this.container).attr('title', usingRuntime);
                $('.plupload_progress_footer', this.container).attr('title', usingRuntime);
                    ...
                    if (details) {
                        ...
                        switch (err.code) {
                            ...
                            case plupload.IMAGE_DIMENSIONS_ERROR:
                                details = plupload.translate('Resolution out of boundaries! <b>%s</b> runtime supports images only up to %wx%hpx.').replace(/%([swh])/g, function ($0, $1) {
                                    switch ($1) {
        ...
        _handleFileStatus: function (file) {
            var actionClass, iconClass, actionHint;
            ...
            switch (file.status) {
                case plupload.DONE:
                    ...
                    actionHint = 'Remove file from queue';
                    break;
        ...
        _updateFileList: function () {
            ...
            $.each(up.files, function (i, file) {
                ...
                filelist.on('click', '#' + file.id + '.plupload_delete .ui-icon, #' + file.id + '.plupload_done .ui-icon', function (e) {
                    $('#' + file.id).remove();
                    up.removeFile(file);
